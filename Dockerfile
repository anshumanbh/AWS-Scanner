...
WORKDIR /go/src/github.com/anshumanbh/awsscanner
...
# Build
COPY main.go ./
RUN go build -v -o ${GOPATH}/bin/awsscanner

# Final Container
FROM alpine:3.7
LABEL maintainer="Anshuman Bhartiya"
COPY --from=build-env /etc/ssl/certs/ca-certificates.crt /etc/ssl/certs/ca-certificates.crt
COPY --from=build-env /go/bin/awsscanner /usr/bin/awsscanner
COPY list.txt ./

ENTRYPOINT ["/usr/bin/awsscanner"]
